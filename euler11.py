...
This one is a straight copy of Jason Hill's http://code.jasonbhill.com/python/project-euler-problem-11/.
His solution was so much better than mine I just ditched mine and learned from his. Thanks, Jason.
"""
...
def main():
	# Convert each string into a substring, splitting on spaces
	M = [i.split() for i in L]

	# Convert the 2-digit substrings into integers, and into a 2-D array
	# There are 20 rows, each containing 20 integers
	M = [[int(j) for j in i] for i in M]

	# The maximum product found to date, which is the answer we want
	max_prod = 0

	for i in range(20):
		for j in range(16): # No need to go to 20
			# right/left products
